...
This page discusses both how URLs and filenames should be handled in
text and guidelines for creating them.
...
1.  The protocol, e.g. `https://`, `http://`, or `ftp://`. The `://` is
    always present.
2.  The domain, e.g. `18f.gsa.gov`. This is “where on the internet” the
    URL is referring to. Also called: “site”, “domain name”, “server”.
3.  Optional: the port, a number specifying how to access the content on
    this particular domain. Not normally used on production sites.
    Follows the domain, separated by a colon, e.g.
    `http://dev.example.com:8080`.
4.  The path: what resource on the domain the URL is referring to. If
    omitted, defaults to `/`, the “root level” of the domain. Always
...
5.  Optional: the query string. Normally contains parameters that have
    been submitted to the site, for example with searches.
    `?q=18f+content+guide` is the query string in
    `https://www.google.com/search?q=18f+content+guide`.
6.  Optional: the fragment. Normally this indicates a specific section
    of a page, but can also be used for other things by
    web applications. `#main` is the fragment in
    `https://pages.18f.gov/content-guide/specific-words-and-phrases/#main`.

Ideally, URLs should be short, memorable, and well-structured. Your
control over your URL may be limited, but you should do what you can
...
In the vast majority of cases, everything user can reach on your site
should have a distinct URL that a user could bookmark and use later to
...
bureaucratic structure, it is unlikely that this is important to users,
...
`18fears.gov` (we would presumably use `ears.18f.gov`).
...
`urls-and-filenames` is part of `content-guide`. If this were a much
larger guide, it's possible that it could be divided further, e.g.
...
Conversely, the use should be able to remove a level from the path and
...
-   Slight improvement:
    ...
    `www` adds nothing for the user.
-   Very slight improvement:
    `long-name-creation-service.departmentoflongnames.bureauofnames.gov`.
    We're assuming that only the lowest level is subject to your control
    in this example.
-   Improvement: `long-name-creation-service.bureauofnames.gov`. The
    user doesn't need the URL to reflect that the service belongs to the
    Department of Long Names.
-   Better: `lncs.bureauofnames.gov`. This change relies on users
    thinking of the service by that abbreviation.
-   Better: `make-long-names.bureauofnames.gov`. If the users aren't
    familiar with that abbreviation, a shorter description of what the
    service does is better to have as the lowest level of the domain.
...
-   Slight improvement:
    ...
    Assuming that creation is the most likely action users will want to
    take, make it the default and strip the requirement for the
    query string.
-   Slight improvement:
    `/services/default/php/forms/departmentoflongnames/longnamecreationservice/`.
    The default page should be delivered automatically, making the
    inclusion of `index.php` unnecessary. Also, hide the technical
    details (`.php`).
-   Improvement: `/departmentoflongnames/longnamecreationservice/`.
    Users don't need to know the technical setup of the site.
-   Improvement:
    `/department-of-long-names/long-name-creation-service/`. Separate
    words with hyphens.
-   Better: `/department-of-long-names/lncs/`. As above, if the users
    are familiar with the abbreviation, use it to make the path shorter.
-   Better: `/department-of-long-names/make-long-names/`. As above, if
    they aren't familiar with the abbreviation, use a shorter
...
-   Better: `make-long-names.bureauofnames.gov`. Reasonable if the
    Department of Long Names has only a couple of services that might be
    popular enough to warrant their own subdomains, particularly if they
    don't necessarily fit cleanly into other content and stand alone.
-   Better:
    `bureauofnames.gov/department-of-long-names/make-long-names/`.
    Reasonable if the Long Name Creation Service is one of many related
    services that the Department of Long Names provides.

### URL shorteners

Worth discussing the advantages of first-party URL shortening?
...
technical work than others. It is most difficult to accomplish when
...
Use the right extension—PDFs should have `.pdf` at the end, JPGs should
have `.jpg` at the end, etc.
...
Whether beginning with protocol or not, do not capitalize. Protocols are
lowercase, and domains are case-insensitive and should be presented as
lowercase. Paths are case-sensitive, however, so their casing must be
preserved.
...
When they are active links, do not include the protocol in the link
text.
